...
class DIAGRAMS_CUSTOM_LOSS(SRData):
    ...
    def __init__(self, args, name='DIAGRAMS_CUSTOM_LOSS', train=True, benchmark=False): #DIAGRAMS_MINI for search, DIAGRAMS for training
        data_range = [r.split('-') for r in args.data_range.split('/')]
        self.begin, self.end = map(int, data_range[0 if train else 1])

        super(DIAGRAMS_CUSTOM_LOSS, self).__init__(
