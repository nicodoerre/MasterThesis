...
def super_resolve_patches(patches, model, args, device="cuda", vis=False, max_vis=5):
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for i in range(N):
            ...
            #patch_tensor = (patch.float() / 255.0).unsqueeze(0).to(device) * args.rgb_range#patch_tensor = transform(patch).unsqueeze(0).to(device) 
            patch_tensor = (patch.float() / 255.0 * args.rgb_range).unsqueeze(0).to(device)
            sr_tensor = model(patch_tensor)[0]  
            ...
            sr_patches_list.append(sr_np)
            # Visualization
            if vis and i < max_vis:
                fig, axes = plt.subplots(1, 2, figsize=(6, 3))
                lr_uint8 = patch.cpu().numpy().transpose(1, 2, 0).round().astype(np.uint8)
                axes[0].imshow(lr_uint8)
...
def super_resolve_full_image(lr_image_np, model, args, device="cuda"):
    """
    Super-resolve a full LR image with an EDSR-family model.

    Parameters
    ----------
    lr_image_np : numpy.ndarray (H × W × 3, uint8, RGB)
        Low-resolution input image.
    model       : torch.nn.Module
        EDSR model that was trained with MeanShift layers.
    args        : argparse.Namespace
        Must contain rgb_range (usually 255).
    device      : str
        "cuda" or "cpu".

    Returns
    -------
    sr_np : numpy.ndarray (H*scale × W*scale × 3, uint8, RGB)
        Super-resolved image.
    """
    model = model.to(device).eval()

    # 1. RGB uint8 → tensor in model’s expected range (0‒rgb_range)
    lr_tensor = to_tensor(lr_image_np).unsqueeze(0).to(device)      # [1,3,H,W] 0‒1
    lr_tensor = lr_tensor * args.rgb_range                          #   → 0‒255

    with torch.no_grad():
        # 2. Forward pass (model internally subtracts / adds mean)
        sr_tensor = model(lr_tensor)[0]                             # [3,H',W']

    # 3. Bring back to 0‒1, then to uint8
    sr_tensor = sr_tensor.clamp(0, args.rgb_range) / args.rgb_range
    sr_np = (sr_tensor.cpu().numpy().transpose(1, 2, 0) * 255.0) \
            .round().astype(np.uint8)                               # HWC uint8
