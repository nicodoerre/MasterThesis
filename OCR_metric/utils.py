...
def get_bounding_boxes(gt_img, min_width=5, min_height=5, vis=True, bb_name=''):
    ...
    for i in range(n_boxes):
        ...
        if raw_conf == '-1':            # drop Tesseract’s non‐word rows
            continue
        conf = int(raw_conf)
        if conf < 70:
            continue 
    ...
    if vis:
        ...
        plt.axis("off")
        plt.savefig(f"C:/Users/nicol/Desktop/UNI/3.Master/MasterThesis/boundingboxes/b{bb_name}.png", bbox_inches="tight", dpi=300)
        plt.show()
...
def compute_pixel_metrics(gt_crop, sr_crop):
    ...
    h, w = gt_np.shape
    # pick the largest odd window ≤ min(h, w), but at least 3
    max_win = min(h, w)
