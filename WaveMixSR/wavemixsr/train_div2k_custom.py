...
def main():
    ...
    #for x2 it was 64
    #for x3 was 48
    PATH = str(args.test_dataset)+'_'+str(args.resolution)+'x_y_Div2k_'+str(args.eval_metric)+'.pth' # for normal training
    ...
    while counter < 25: #was 25, 3 for search
        ...
        with tqdm(trainloader, unit="batch") as tepoch:
            ...
            for data in tepoch:
                ...
                if isinstance(criterion, Custom_Loss):
                ...
                scaler.update()
                sr_y_clamped = torch.clamp(sr_y, 0.0, 1.0)
                gt_y_clamped = torch.clamp(gt_y, 0.0, 1.0)
                epoch_PSNR = psnr(sr_y_clamped, gt_y_clamped)
                epoch_SSIM = structural_similarity_index_measure(sr_y_clamped, gt_y_clamped)
        ...
        with torch.no_grad():
            for data in valloader:
                ...
                outputs = outputs[:, 0:1, :, :]
                labels = labels[:, 0:1, :, :]
                outputs_y = outputs.clamp(0.0, 1.0)
                labels_y  = labels.clamp(0.0, 1.0)

                PSNR += psnr(outputs_y, labels_y) / len(valloader)
                sim += structural_similarity_index_measure(outputs_y, labels_y) / len(valloader)
    ...
    while counter < 25:  # loop over the dataset multiple times, was 25, 3 for search
        ...
        with tqdm(trainloader, unit="batch") as tepoch:
            ...
            for data in tepoch:
              ...
              gt_y = torch.clamp(gt_y, 0.0, 1.0)
              epoch_PSNR = psnr(sr_y, gt_y) 
              epoch_SSIM = structural_similarity_index_measure(sr_y, gt_y)
              epoch_loss += loss / len(trainloader)
              tepoch.set_postfix_str(f" loss : {epoch_loss:.4f} - PSNR: {epoch_PSNR:.4f} - SSIM: {epoch_SSIM:.4f}" )
        ...
        with torch.no_grad():
            for data in testloader:
                ...
                PSNR += psnr(outputs_y, labels_y) / len(testloader)
                sim += structural_similarity_index_measure(outputs_y, labels_y) / len(testloader)
